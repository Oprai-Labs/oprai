...
pub struct RaydiumOpenPositionParams {
    ...
    #[serde(default, deserialize_with = "crate::services::params::lenient_opt")]
    pub slippage_bps: Option<u32>,
    /// Position NFT mint, generated by the CLIENT.
    ///
    /// Not something the model ever fills in — the frontend adds it so the
    /// wallet can sign before the mint does. Absent means an older client,
    /// and the builder falls back to generating and pre-signing the key
    /// itself. See `raydium_clmm::builder::OpenPositionRequest`.
    #[serde(default)]
...
pub async fn build_raydium_open_position(
    ...
    // The browser generates the position NFT key and sends us its pubkey, so
    // the wallet can be the first signature on the transaction. Absent (an
    // older client), the builder falls back to generating and pre-signing.
    let position_nft_mint = params
        .position_nft_mint
        .as_deref()
        .map(solana_sdk::pubkey::Pubkey::from_str)
        .transpose()
        .map_err(|e| AppError::InvalidParams(format!("invalid positionNftMint: {e}")))?;
