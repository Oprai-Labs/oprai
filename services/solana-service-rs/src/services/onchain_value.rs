...
use crate::services::fees::{USDC_MINT, USDT_MINT};
...
pub async fn confirmed_trade_notional_usd(
    ...
    let deltas = tokio::task::spawn_blocking(move || -> Option<Vec<(String, f64)>> {
        ...
        Some(wallet_token_deltas(&meta, &wallet_owned))
    })
    .await
    .ok()??;
    ...
    // Stable sides price exactly at $1; the rest go to Jupiter's price API in a
    // single batched call. Take the max USD magnitude across all sides.
    let mut best = 0.0_f64;
    let mut to_price: Vec<(String, f64)> = Vec::new();
    for (mint, amt) in &deltas {
