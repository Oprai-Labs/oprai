...
pub struct JitoUnstakeParams {
    ...
    #[serde(default, deserialize_with = "crate::services::params::lenient_opt")]
    pub slippage_bps: Option<u32>,
    /// Address of the stake account that receives the unstaked funds,
    /// generated by the CLIENT so the wallet can sign before it does. Only
    /// the standard (non-instant) path creates an account; the instant path
    /// pays out from the reserve and needs no extra signer. Absent means an
    /// older client and we pre-sign as before.
    #[serde(default)]
...
fn build_unstake_transaction(
    ...
    client_stake_pubkey: Option<Pubkey>,
) -> Result<JitoStakeResult, AppError> {
    ...
    // The new stake account that receives the unstaked funds — the user owns
    // it after the transaction. Client-supplied when the browser holds the
    // key (so the wallet signs first); generated here as the legacy fallback.
    let new_stake_kp: Option<Keypair> = match client_stake_pubkey {
        Some(_) => None,
        None => Some(Keypair::new()),
    };
    let new_stake_pubkey = client_stake_pubkey
    ...
    // Only on the legacy path. With a client-held key both signer slots stay
    // empty so the wallet is the first signature on the transaction.
