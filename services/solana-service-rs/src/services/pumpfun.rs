...
pub fn build_launch_token_transaction_blocking(
    ...
    // Compute budget — create_v2 + ATA create + buy together need ~500k CU;
    // add extra headroom when cashback init is included.
    let has_initial_buy = params.initial_buy_amount
        .as_ref()
        .and_then(|s| s.parse::<f64>().ok())
        .unwrap_or(0.0) > 0.0;
    let compute_units: u64 = if has_initial_buy { 700_000 } else { 400_000 };
    ...
    if initial_buy_sol > 0.0 {
        ...
        // New tokens are always Token-2022 (create_v2)
        let create_ata_ix = build_create_ata_instruction(creator_pubkey, &mint_pubkey, &token_2022_program());
        ...
        // Fetch live fee recipient from pump.fun global account so simulation matches on-chain state.
        // Falls back to the hardcoded address if the RPC call fails.
        let fee_recipient = {
            let global = find_global_pda();
            const FEE_RECIPIENT_OFFSET: usize = 41;
            match rpc.client().get_account(&global) {
                Ok(acc) if acc.data.len() >= FEE_RECIPIENT_OFFSET + 32 => {
                    Pubkey::try_from(&acc.data[FEE_RECIPIENT_OFFSET..FEE_RECIPIENT_OFFSET + 32])
                        .unwrap_or_else(|_| Pubkey::from_str(PUMP_FUN_FEE_RECIPIENT_FALLBACK).unwrap())
                }
                _ => Pubkey::from_str(PUMP_FUN_FEE_RECIPIENT_FALLBACK).unwrap(),
            }
        };
    ...
    // Step 7: Sign with mint keypair only when backend generated it.
...
pub async fn build_pumpfun_buy(
    ...
    // Graduated tokens (complete: true) must go through PumpSwap AMM, not the bonding curve.
    if coin.get("complete").and_then(|v| v.as_bool()).unwrap_or(false) {
        tracing::info!(mint = %mint, "Token graduated — redirecting pumpfun_buy → pumpswap_buy");
        return build_pumpswap_buy(http, rpc, wallet, params).await;
    }
...
pub async fn build_pumpfun_sell(
    ...
    // Graduated tokens must go through PumpSwap AMM.
    if coin.get("complete").and_then(|v| v.as_bool()).unwrap_or(false) {
        tracing::info!(mint = %mint, "Token graduated — redirecting pumpfun_sell → pumpswap_sell");
        return build_pumpswap_sell(http, rpc, wallet, params).await;
    }
...
pub async fn build_pumpswap_buy(
    ...
    let (pool_data_result, global_config_result) = tokio::join!(
    ...
    tracing::info!(
        ...
        is_cashback_coin, "PumpSwap pool data read from chain"
    );
    ...
    // Create base and quote (wSOL legacy SPL) ATAs
    // PumpSwap graduated tokens use Token-2022
    let sol_mint = Pubkey::from_str(SOL_MINT).expect("valid");
    let create_base_ata_ix = build_create_ata_instruction(&buyer, &mint, &token_2022_program());
...
pub async fn build_pumpswap_sell(
    ...
    tracing::info!(
        pool = %pool, coin_creator = %coin_creator,
        pool_base_vault = %pool_base_vault, pool_quote_vault = %pool_quote_vault,
        is_cashback_coin, "PumpSwap pool data read from chain for sell"
    );
