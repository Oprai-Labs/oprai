...
export interface DammV2AddParams {
  ...
  slippageBps?: number;
  /**
   * Position NFT mint, generated by the CLIENT so the wallet can sign before
   * it does. Only read when opening a position; adding to an existing one
   * mints nothing. Absent means an older client and we generate + sign here.
   */
...
export async function buildDammV2AddLiquidity(
  ...
  let clientHoldsPositionNft = false;
  if (params.position) {
    ...
    // must sign. Who holds that key decides the signing ORDER: when the browser
    // generated it, we build with its public key and sign nothing, so the
    // wallet is the first signature on the transaction — which is what
    // Phantom's multi-signer guidance asks for. A transaction that arrives
    // already carrying a stranger's signature is the shape its scanner cannot
    // vouch for.
    //
    // Absent (an older client) we fall back to generating and signing it here:
    // the previous behaviour, kept working rather than kept preferred.
    const clientNft =
      typeof params.positionNftMint === "string" && params.positionNftMint.trim()
        ? new PublicKey(params.positionNftMint.trim())
        : null;
    const positionNft = clientNft ? null : Keypair.generate();
    tx = await amm.createPositionAndAddLiquidity({
      owner: user,
      pool: poolPk,
      positionNft: clientNft ?? positionNft!.publicKey,
      liquidityDelta: deposit.liquidityDelta,
      maxAmountTokenA: maxA,
      maxAmountTokenB: maxB,
      tokenAAmountThreshold: maxA,
    ...
    if (positionNft) localSigners.push(positionNft);
