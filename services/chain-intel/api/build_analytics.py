...
async def launches() -> None:
    ...
    SELECT
      ...
      multiIf(p.curve != '', 'Pons',
              ctx.callee IN ('{routers_in}'), CASE ctx.callee {routers_case} ELSE '' END,
              h.hooks != '' AND (ap.blk = 0 OR h.blk <= ap.blk), CASE h.hooks {hooks_case} ELSE 'hook' END,
              ap.pool != '', 'direct pool', 'unknown') AS launchpad,
      multiIf(p.curve != '', 'factory', ctx.callee IN ('{routers_in}'), 'creation router',
              h.hooks != '' AND (ap.blk = 0 OR h.blk <= ap.blk), 'uniswap-v4 hook', ap.pool != '', 'first pool', '') AS launchpad_source,
...
def stats_sql(where_tokens: str) -> str:
    ...
    WITH
      ...
      px AS (SELECT token, maxIf(vwap, v >= 20) AS ath, argMaxIf(hour, vwap, v >= 20) AS ath_hour,
                    argMinIf(vwap, hour, v >= 20) AS first_px, argMaxIf(vwap, hour, v >= 0.5) AS last_px,
